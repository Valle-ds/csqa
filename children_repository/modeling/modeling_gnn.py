...
class Decoder(nn.Module):
    def __init__(self, args, num_rels, h_dim):
        ...
        self.adversarial_temperature = (
            args.link_negative_adversarial_sampling_temperature
        )
    ...
    def loss(self, scores):
        ...
        loss = (
            positive_sample_loss + negative_sample_loss
        ) / 2 + self.reg_param * self.reg_loss()
...
class TransEDecoder(Decoder):
    def __init__(self, args, num_rels, h_dim, dist_func='l2'):
        ...
        print(
            f'Initializing w_relation for TransEDecoder... (gamma={self.gamma})',
            file=sys.stderr,
        )
        self.epsilon = 2.0
    ...
    def __repr__(self):
        return f'{self.__class__.__name__}(embedding_size={self.embedding_dim}, num_relations={self.num_relations}, gamma={self.gamma}, dist_ord={self.dist_ord})'
...
class DistMultDecoder(Decoder):
    ...
    def __repr__(self):
        return f'{self.__class__.__name__}(embedding_size={self.embedding_dim}, num_relations={self.num_relations})'
...
class RotatEDecoder(Decoder):
    def __init__(self, args, num_rels, h_dim):
        ...
        print(
            f'Initializing w_relation for RotatEDecoder... (gamma={self.gamma})',
            file=sys.stderr,
        )
        self.epsilon = 2.0
        self.register_parameter(
            'w_relation',
            nn.Parameter(torch.Tensor(self.num_relations, self.embedding_dim // 2)),
        )
        self.embedding_range = (self.gamma + self.epsilon) / self.embedding_dim
        with torch.no_grad():
            self.w_relation.uniform_(-self.embedding_range, self.embedding_range)
    ...
    def __repr__(self):
        return f'{self.__class__.__name__}(embedding_size={self.embedding_dim}, num_relations={self.num_relations}, gamma={self.gamma})'
