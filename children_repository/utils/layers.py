...
class TypedLinear(nn.Linear):
    ...
    def forward(self, X: torch.Tensor, type_ids: Optional[torch.Tensor] = None):
        """
...
class MaxPoolLayer(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, mask_or_lengths: torch.Tensor):
        """
...
class MatrixVectorScaledDotProductAttention(nn.Module):
    ...
    def forward(
        ...
        if Qmax > Kmax:
            attn = ((q.float().unsqueeze(1) / self.temperature) * k.float()).sum(
                2
            )  # (n*b, l)
        else:
            attn = (q.float().unsqueeze(1) * (k.float() / self.temperature)).sum(
                2
            )  # (n*b, l)
