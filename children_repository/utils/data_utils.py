...
try:
    from transformers import ALBERT_PRETRAINED_CONFIG_ARCHIVE_MAP, AlbertTokenizer
except ImportError:
...
class MultiGPUSparseAdjDataBatchGenerator(object):
    ...
    def __init__(
        ...
        self.span_len_dist = [
            self.geo_p * (1 - self.geo_p) ** (i - self.span_len_lower)
            for i in range(self.span_len_lower, self.span_len_upper + 1)
        ]
        self.span_len_dist = [x / (sum(self.span_len_dist)) for x in self.span_len_dist]

        self.eval_end_task_mode = (
            False  # if True, use the non-modified text and KG inputs
        )
    ...
    def __iter__(self):
        ...
        for a in range(0, n, bs):
            ...
            batch_tensors0 = [
                self._to_device(x[batch_indexes], self.device1) for x in self.tensors0
            ]
            assert (
                len(batch_tensors0) == 4
            )  # tensors0: all_input_ids, all_input_mask, all_segment_ids, all_output_mask
            batch_lm_inputs, batch_lm_labels = self.process_lm_data(batch_tensors0)

            batch_tensors1 = [
                self._to_device(x[batch_indexes], self.device1) for x in self.tensors1
            ]
            assert (
                len(batch_tensors1) == 5
            )  # tensors1: concept_ids, node_type_ids, node_scores, adj_lengths, special_nodes_mask
            batch_tensors1[0] = batch_tensors1[0].to(self.device0)
            batch_lists0 = [
                self._to_device([x[i] for i in batch_indexes], self.device0)
                for x in self.lists0
            ]
            batch_lists1 = [
                self._to_device([x[i] for i in batch_indexes], self.device1)
                for x in self.lists1
            ]
            ...
            edge_index = self._to_device(
                [edge_index_all[i] for i in batch_indexes], self.device1
            )
            edge_type = self._to_device(
                [edge_type_all[i] for i in batch_indexes], self.device1
            )
            node_type_ids = batch_tensors1[1]  # [bs, nc, n_nodes]
            assert node_type_ids.dim() == 3
            edge_index, edge_type, pos_triples, neg_nodes = self.process_graph_data(
                edge_index, edge_type, node_type_ids
            )
    ...
    def process_lm_data(self, batch_tensors0):
        ...
        batch_lm_inputs, batch_lm_labels = self.mask_tokens(
            inputs=_inputs,
    ...
    def _word_mask(self, input_tokens: List[str], max_predictions=512):
        ...
        num_to_predict = min(
            max_predictions,
            max(1, int(round(effective_num_toks * self.mlm_probability))),
        )
        masked_lms = []
        covered_indexes = set()
    ...
    def process_graph_data(self, edge_index, edge_type, node_type_ids):
        ...
        for bid in range(bs):
            for cid in range(nc):
                ...
                _edge_index, _edge_type, _pos_triples, _neg_nodes = (
                    self._process_one_graph(_edge_index, _edge_type, _node_type_ids)
                )
        ...
        input_edge_index = list(
            map(list, zip(*(iter(input_edge_index),) * nc))
        )  # nested list of shape (n_samples, num_choice)
    ...
    def _process_one_graph(self, _edge_index, _edge_type, _node_type_ids):
        ...
        positions = positions[~is_special.to('cpu')]  # [some_E, ]
        drop_count = min(
            self.args.link_drop_max_count,
            int(len(positions) * self.args.link_drop_probability),
        )
        if len(positions) > 0 and drop_count > 0:
            drop_idxs = torch.multinomial(
                torch.full((len(positions),), 1.0), drop_count, replacement=False
            )  # [drop_count, ]
        else:
        ...
        mask = (
            mask.index_fill_(dim=0, index=drop_positions, value=1).bool().to(device)
        )  # [E, ]

        real_drop_count = int(
            drop_count * (1 - self.args.link_drop_probability_in_which_keep)
        )
        real_drop_positions = positions[
            drop_idxs[:real_drop_count]
        ]  # [real_drop_count, ]
        real_mask = torch.zeros((E,)).long()  # [E, ]
        real_mask = (
            real_mask.index_fill_(dim=0, index=real_drop_positions, value=1)
            .bool()
            .to(device)
        )  # [E, ]
        ...
        neg_nodes = torch.randint(
            0, effective_num_nodes, (num_edges, num_corruption), device=device
        )  # [n_triple, n_neg]
        return input_edge_index, input_edge_type, pos_triples, neg_nodes
...
class DRAGON_DataLoader(object):
    def __init__(
        ...
        if args.mode == 'train':
            ...
            *self.train_decoder_data, self.train_adj_data = (
                self.load_sparse_adj_data_with_contextnode(
                    train_adj_path,
                    max_node_num,
                    train_concepts_by_sents_list,
                    mode='train',
            ...
            if not debug:
                for i, x in enumerate(
                    ...
                    ), f'{len(self.train_qids)}, {len(self.train_adj_data[0])}, {x.size(0)}, {i}'

            # Optionally we can subsample the training set.
            assert 0.0 < subsample <= 1.0
            if subsample < 1.0 or n_train >= 0:
        ...
        # Load dev data
        if args.mode == 'train' and dev_statement_path and dev_adj_path:
            (
                self.dev_qids,
                self.dev_labels,
                self.dev_encoder_data,
                dev_concepts_by_sents_list,
            ) = self.load_input_tensors(dev_statement_path, max_seq_length)

            *self.dev_decoder_data, self.dev_adj_data = (
                self.load_sparse_adj_data_with_contextnode(
                    dev_adj_path, max_node_num, dev_concepts_by_sents_list
                )
            )
            if not debug:
                assert all(
                    len(self.dev_qids) == len(self.dev_adj_data[0]) == x.size(0)
                    for x in [self.dev_labels]
                    + self.dev_encoder_data
        ...
        # Load test data
        if args.mode == 'eval' and test_statement_path and test_adj_path:
            (
                self.test_qids,
                self.test_labels,
                self.test_encoder_data,
                test_concepts_by_sents_list,
            ) = self.load_input_tensors(test_statement_path, max_seq_length)
            ...
            *self.test_decoder_data, self.test_adj_data = (
                self.load_sparse_adj_data_with_contextnode(
                    test_adj_path, max_node_num, test_concepts_by_sents_list
                )
            )
            if not debug:
                assert all(
    ...
    def train_size(self):
        return len(self.train_qids) if hasattr(self, 'train_qids') else 0

    def dev_size(self):
        return len(self.dev_qids) if hasattr(self, 'dev_qids') else 0
    ...
    def load_resources(self, kg):
        ...
        if kg == 'cpnet':
            ...
            self.id2relation = [
                r.strip()
                for r in open(
                    f'{os.path.dirname(self.args.kg_vocab_path)}/relations.txt'
                )
            ]
        else:
    ...
    def load_input_tensors(self, input_jsonl_path, max_seq_length, mode='eval'):
                ...
                try:
                    while True:
                        ...
                        if isinstance(obj, dict):
                            ...
                            assert (
                                len(obj) == 4
                            )  # example_ids, all_label, data_tensors, concepts_by_sents_list
                            input_tensors = obj
                        else:
        ...
        if mode == 'train' and self.args.local_rank != -1:
            example_ids, all_label, data_tensors, concepts_by_sents_list = (
                input_tensors  # concepts_by_sents_list is always []
            )
            ...
            if rem != 0:
                ...
                all_label = torch.cat(
                    [all_label, all_label[: self.args.world_size - rem]], dim=0
                )
                data_tensors = [
                    torch.cat([t, t[: self.args.world_size - rem]], dim=0)
            ...
            _select = (
                torch.arange(total_num_aim) % self.args.world_size
            ) == self.args.local_rank  # bool tensor
    ...
    def load_sparse_adj_data_with_contextnode(
        self, adj_pk_path, max_node_num, concepts_by_sents_list, mode='eval'
    ):
        """Construct input tensors for the GNN component of the model."""
        print('Loading sparse adj data...')
        cache_path = (
            adj_pk_path
            + '-nodenum{}'.format(max_node_num)
            + ('-cntsall' if self.cxt_node_connects_all else '')
            + '.loaded_cache'
        )
        ...
        if use_cache:
            ...
            ori_adj_sigma = np.sqrt(
                ((adj_lengths_ori.float() - ori_adj_mean) ** 2).mean().item()
            )
            print(
                '| ori_adj_len: mu {:.2f} sigma {:.2f} | adj_len: {:.2f} |'.format(
                    ori_adj_mean, ori_adj_sigma, adj_lengths.float().mean().item()
                )
                + ' prune_rate： {:.2f} |'.format(
                    (adj_lengths_ori > adj_lengths).float().mean().item()
                )
                + ' qc_num: {:.2f} | ac_num: {:.2f} |'.format(
            ...
            edge_index = list(
                map(list, zip(*(iter(edge_index),) * self.num_choice))
            ...
            edge_type = list(
                map(list, zip(*(iter(edge_type),) * self.num_choice))
            ...
            (
                ...
                x.view(-1, self.num_choice, *x.size()[1:])
                for x in (
                    adj_lengths_ori,
                    concept_ids,
                    node_type_ids,
            ...
            if mode == 'train' and self.args.local_rank != -1:
                assert (
                    ...
                    == len(edge_index)
                    == len(edge_type)
                    == len(special_nodes_mask)
                )  # they equal to n_questions * num_choice
                _ts = [
                    adj_lengths_ori,
                    concept_ids,
                    node_type_ids,
            ...
            idx = -1
            for _idx, (_data, _cpts_by_sents) in tqdm(
                enumerate(zip(adj_concept_pairs, concepts_by_sents_list)),
                total=n_questions * self.num_choice,
                ...
                num_concept = min(
                    len(concepts) + n_special_nodes, actual_max_node_num
                ...
                concept_ids[idx, 0] = (
                    context_node  # this is the "concept_id" for contextnode
                )
                ...
                # Load adj
                ij = torch.tensor(
                    adj.row, dtype=torch.int64
                )  # (num_matrix_entries, ), where each entry is coordinate
                k = torch.tensor(
                    adj.col, dtype=torch.int64
                )  # (num_matrix_entries, ), where each entry is coordinate
                n_node = adj.shape[1]
                if n_node > 0:
                ...
                for _coord, q_tf in enumerate(qm):
                    ...
                    if q_tf:
                        extra_i.append(
                            cxt2qlinked_rel
                        )  # rel from contextnode to question concept
                        ...
                        extra_i.append(
                            cxt2other_rel
                        )  # rel from contextnode to other concept
                        extra_j.append(0)  # contextnode coordinate
                ...
                for _coord, a_tf in enumerate(am):
                    ...
                    if a_tf:
                        extra_i.append(
                            cxt2alinked_rel
                        )  # rel from contextnode to answer concept
                        extra_j.append(0)  # contextnode coordinate
                ...
                if self.args.max_num_relation > 0:
                    ...
                    assert _keep.dim() == 1 and _keep.size(0) == i.size(0) == j.size(
                        0
                    ) == k.size(0)
                    i = i[_keep]
                    j = j[_keep]
...
def load_gpt_input_tensors(statement_jsonl_path, max_seq_length):
    ...
    def load_qa_dataset(dataset_path):
        ...
        with open(dataset_path, 'r', encoding='utf-8') as fin:
            ...
            for line in fin:
                ...
                output.append((
                    input_json['id'],
                    input_json['question']['stem'],
                    *[ending['text'] for ending in input_json['question']['choices']],
                    label,
                ))
        return output
    ...
    def pre_process_datasets(
        ...
        for dataset in encoded_datasets:
            ...
            lm_labels = np.full(
                (n_batch, num_choices, max_seq_length), fill_value=-1, dtype=np.int64
            )
            ...
            for (
                ...
                for j in range(len(choices)):
                    ...
                    qa = (
                        [start_token] + q + [delimiter_token] + choices[j] + [clf_token]
                    )
    ...
    ((input_ids, mc_token_ids, lm_labels, mc_labels),) = pre_process_datasets(
        [encoded_dataset], num_choices, max_seq_length, *special_tokens_ids
    )
        ...
        for line in tqdm(f.readlines()):
            ...
            examples.append(
                InputExample(
                    ...
                    question='',
                    endings=[
                        ending['text'] for ending in json_dic['question']['choices']
                    ],
                    label=label,
                )
            )
...
def simple_convert_examples_to_features(
    examples,
    label_list,
    max_seq_length,
    tokenizer,
    ...
    for ex_index, example in tqdm(
        ...
        if debug and ex_index >= debug_sample_size:
            break
        choices_features = []
        for _ending_idx, (context, ending) in enumerate(
...
def load_bert_xlnet_roberta_input_tensors(
    statement_jsonl_path, max_seq_length, debug, tokenizer, debug_sample_size
):
    ...
    def convert_features_to_tensors(features):
        ...
        all_output_mask = torch.tensor(
            select_field(features, 'output_mask'), dtype=torch.bool
        )
    ...
    if num_workers <= 1:
        ...
        examples_list = [
            examples[i * c_size : (i + 1) * c_size] for i in range(num_workers)
        ]
        label_list_list = [
            list(range(len(examples[0].endings))) for i in range(num_workers)
        ]
        ...
        with Pool(num_workers) as p:
            ress = list(
                p.starmap(
                    simple_convert_examples_to_features,
                    zip(
                        examples_list,
                        label_list_list,
                        max_seq_length_list,
