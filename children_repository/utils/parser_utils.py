...
def add_data_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--data_dir", default="data", type=str, help="Path to the data directory"
    )
    # statements
    parser.add_argument(
        "--train_statements",
        default="{data_dir}/{dataset}/statement/train.statement.jsonl",
    )
    parser.add_argument(
        "--dev_statements", default="{data_dir}/{dataset}/statement/dev.statement.jsonl"
    )
    parser.add_argument(
        "--test_statements",
        default="{data_dir}/{dataset}/statement/test.statement.jsonl",
    )
    ...
    for split in data_splits:
        for attribute in ("statements",):
            ...
            parser.set_defaults(
                **{
                    attr_name: getattr(args, attr_name).format(
...
def add_encoder_arguments(parser: argparse.ArgumentParser):
    parser.add_argument(
        "-enc", "--encoder", default="bert-large-uncased", help="encoder type"
    )
    parser.add_argument(
        "--encoder_load_path", default="", help="custom encoder to load"
    )
    parser.add_argument(
        "--encoder_layer",
        default=-1,
    ...
    parser.add_argument(
        "-elr", "--encoder_lr", default=1e-8, type=float, help="learning rate"
    )
    args, _ = parser.parse_known_args()
...
def add_optimization_arguments(parser: argparse.ArgumentParser):
    parser.add_argument(
        ...
        choices=["margin_rank", "cross_entropy"],
        help="model type",
    )
    parser.add_argument(
        "--optim",
    ...
    parser.add_argument(
        "--lr_schedule",
        default="fixed",
        choices=["fixed", "warmup_linear", "warmup_constant"],
        help="learning rate scheduler",
    )
    parser.add_argument("-bs", "--batch_size", default=32, type=int)
    parser.add_argument("--warmup_steps", type=float, default=150)
    parser.add_argument(
        "--max_grad_norm", default=1.0, type=float, help="max grad norm (0 to disable)"
    )
    parser.add_argument(
        "--weight_decay", default=1e-2, type=float, help="l2 weight decay strength"
    )
    parser.add_argument(
        "--n_epochs",
        default=100,
