...
WarmupConstantSchedule: types.ModuleType
try:
...
def load_data(args, devices, kg):
    ...
    if args.local_rank != -1:
        _seed = (
            args.seed + (2**args.local_rank - 1)
        )  # use different seed for different gpu process so that they have different training examples
...
def construct_model(args, kg, dataset):
    ...
    if kg == 'cpnet':
        ...
        print(
            'final_num_relation',
            dataset.final_num_relation,
            'len(id2relation)',
            len(dataset.id2relation),
        )
        print(
            'final_num_relation',
            dataset.final_num_relation,
    ...
    encoder_load_path = (
        args.encoder_load_path if args.encoder_load_path else args.encoder
    )
    model = modeling_dragon.DRAGON(
        args,
        encoder_load_path,
        k=args.k,
        n_ntype=n_ntype,
        ...
        ie_dim=args.ie_dim,
        info_exchange=args.info_exchange,
        ie_layer_num=args.ie_layer_num,
        sep_ie_layers=args.sep_ie_layers,
...
def count_parameters(loaded_params, not_loaded_params):
    ...
    num_fixed_params = sum(
        p.numel() for p in not_loaded_params.values() if not p.requires_grad
    )
...
def calc_eval_accuracy(
    ...
    with torch.no_grad():
        for qids, labels, *input_data in tqdm(eval_set, desc='Dev/Test batch'):
            ...
            end_loss, n_corrects = calc_loss_and_acc(
                logits, labels, loss_type, loss_func
            )
            link_loss, pos_link_loss, neg_link_loss = link_losses
            loss = (
                args.end_task * end_loss
                + args.mlm_task * mlm_loss
                + args.link_task * link_loss
            )
            ...
            if save_test_preds:
                ...
                for qid, pred in zip(qids, predictions):
                    print(
                        '{},{}'.format(qid, chr(ord('A') + pred.item())), file=f_preds
                    )
                    f_preds.flush()
            if debug:
                break
...
def create_miterator(qa_dataloader):
    qa_d = qa_dataloader
...
def train(args, resume, devices, kg):
        ...
        if args.local_rank in [-1, 0]:
            ...
            if not resume:
                with open(log_path, 'w') as fout:
                    fout.write(
                        'epoch,step,train_acc,dev_acc,test_acc,best_dev_acc,final_test_acc,best_dev_epoch\n'
                    )
    ...
    (
        ...
        small_lr_params,
        large_lr_params,
    ) = sep_params(model, loaded_roberta_keys)
    ...
    grouped_parameters = [
        {
            'params': [
                p
                for n, p in small_lr_params.items()
        ...
        {
            'params': [
                p for n, p in small_lr_params.items() if any(nd in n for nd in no_decay)
            ],
            'weight_decay': 0.0,
            'lr': args.encoder_lr,
        },
        {
            'params': [
                p
                for n, p in large_lr_params.items()
                if not any(nd in n for nd in no_decay)
            ],
            'weight_decay': args.weight_decay,
            'lr': args.decoder_lr,
        ...
        {
            'params': [
                p for n, p in large_lr_params.items() if any(nd in n for nd in no_decay)
            ],
            'weight_decay': 0.0,
            'lr': args.decoder_lr,
        },
    ...
    for epoch_id in trange(
        0, args.n_epochs, desc='Epoch', disable=args.local_rank not in [-1, 0]
    ):  # trange(last_epoch + 1, args.n_epochs, desc="Epoch"):
        ...
        trloader = dataset.train(
            steps=args.redef_epoch_steps, local_rank=args.local_rank
        )

        loader = create_miterator(trloader)()

        sizes = []
        if trloader is not None:
            sizes.append(len(trloader))
        #
        for _i, (qa_data) in tqdm(
            enumerate(loader),
            desc='Batch',
            ...
            for _idx_, a in enumerate(a_list):
                ...
                if args.fp16:
                    with torch.cuda.amp.autocast():
                        ...
                        logits, mlm_loss, link_losses = model(*[
                            x[a:b] for x in input_data
                        ])  # logits: [bs, nc]
                        end_loss, n_corrects = calc_loss_and_acc(
                            logits, labels[a:b], args.loss, loss_func
                        )
                else:
                    if input_data[0][a:b].size()[0] == 0:
                        break
            ...
            if (global_step + 1) % args.log_interval == 0:
                ...
                if args.local_rank in [-1, 0]:
                    print(
                        '| step {:5} |  lr: {:9.7f} | total loss {:7.4f} | ms/batch {:7.2f} |'.format(
                            global_step,
                            scheduler.get_lr()[0],
        ...
        if args.local_rank in [-1, 0]:
            ...
            preds_path = os.path.join(
                args.save_dir, 'dev_e{}_preds.csv'.format(epoch_id)
            )
            ...
            if dev_dataloader is not None:
                ...
                if args.end_task and (args.mlm_task or args.link_task):
                    ...
                    _, dev_end_loss, _, _, _, _, dev_acc = calc_eval_accuracy(
                        args,
                        dev_dataloader,
                        model,
                        args.loss,
            ...
            print('-' * 71)
            print(
                '| epoch {:3} | step {:5} | dev_acc {:7.4f} |'.format(
                    epoch_id, global_step, dev_acc
                )
            )
            print('-' * 71)
            ...
            wandb.log(
                {
                    ...
                    'dev_end_loss': dev_end_loss,
                    'dev_mlm_loss': dev_mlm_loss,
                    'dev_link_loss': dev_link_loss,
                    'dev_pos_link_loss': dev_pos_link_loss,
                    'dev_neg_link_loss': dev_neg_link_loss,
            ...
            if (args.save_model == 2) or (
                (args.save_model == 1) and (best_dev_epoch == epoch_id)
            ):
                if args.local_rank != -1:
...
def get_devices(args):
    ...
    print(
        ...
        % (args.local_rank, device0, bool(args.local_rank != -1), world_size),
        file=sys.stderr,
    )
...
def main(args):
    ...
    if args.local_rank in [-1, 0]:
        wandb_id = (
            args.resume_id
            if resume and (args.resume_id not in [None, 'None'])
        ...
        print(
            'screen: %s'
            % subprocess.check_output('echo $STY', shell=True).decode('utf')
        )
        print(
            'gpu: %s'
            % subprocess.check_output('echo $CUDA_VISIBLE_DEVICES', shell=True).decode(
                'utf'
            )
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--use_codalab', default=0, type=int, help='using codalab or not'
    )
    parser.add_argument(
        '--save_dir', default='./saved_models/', help='model output directory'
    )
    parser.add_argument(
        '--save_model',
        default=2,
    ...
    parser.add_argument(
        '--load_model_path',
        default=None,
        help='The model checkpoint to load in the evaluation mode.',
    )
    parser.add_argument(
        '-h',
        '--help',
    ...
    parser.add_argument(
        '--run_name', required=True, type=str, help='The name of this experiment run.'
    )
    parser.add_argument(
        '--resume_checkpoint',
        default=None,
    ...
    parser.add_argument(
        '--world_size', type=int, default=1, help='For distributed training: world_size'
    )
    parser.add_argument(
        '--data_loader_one_process_at_a_time', default=False, type=utils.bool_flag
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help='Task weight for the end task (MCQA)',
    )
    parser.add_argument(
        '--mlm_task', type=float, default=0.0, help='Task weight for the MLM task'
    )
    parser.add_argument(
        '--link_task', type=float, default=0.0, help='Task weight for the LinkPred task'
    )
    ...
    parser.add_argument(
        '--link_drop_probability_in_which_keep',
        type=float,
        default=0.2,
    ...
    parser.add_argument(
        '--link_negative_adversarial_sampling',
        type=utils.bool_flag,
        default=True,
        help='',
    )
    parser.add_argument(
        '--link_negative_adversarial_sampling_temperature',
        type=float,
        default=1,
        help='',
    )
    ...
    parser.add_argument(
        '--link_proj_headtail', type=utils.bool_flag, default=False, help=''
    )
    parser.add_argument(
        '--scaled_distmult', type=utils.bool_flag, default=False, help=''
    )
    ...
    parser.add_argument(
        ...
        default=-1,
        type=int,
        help='max number of KG relation types to keep.',
    )
    parser.add_argument(
        '--kg_only_use_qa_nodes', default=False, type=utils.bool_flag, help=''
    )

    parser.add_argument(
        '--train_adj',
        default=f'{args.data_dir}/{args.dataset}/graph/train.graph.adj.pkl',
        help='The path to the retrieved KG subgraphs of the training set.',
    )
    parser.add_argument(
        '--dev_adj',
        default=f'{args.data_dir}/{args.dataset}/graph/dev.graph.adj.pkl',
        help='The path to the retrieved KG subgraphs of the dev set.',
    )
    parser.add_argument(
        '--test_adj',
        default=f'{args.data_dir}/{args.dataset}/graph/test.graph.adj.pkl',
    ...
    parser.add_argument(
        ...
        default=1.0,
        type=float,
        help='The ratio to subsample the training set.',
    )
    parser.add_argument(
        '--n_train',
        default=-1,
    ...
    # Model architecture
    parser.add_argument(
        '-k', '--k', default=5, type=int, help='The number of Fusion layers'
    )
    parser.add_argument(
        '--att_head_num',
        default=2,
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help='number of hidden layers of the final MLP',
    )
    parser.add_argument(
        '--freeze_ent_emb',
        default=True,
    ...
    parser.add_argument(
        ...
        default=200,
        type=int,
        help='number of the hidden units of the MInt operator.',
    )
    parser.add_argument(
        '--residual_ie', default=0, type=int, help='Whether to use residual MInt.'
    )
    parser.add_argument(
        '--info_exchange',
        default=True,
        choices=[True, False, 'every-other-layer'],
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help='number of hidden layers in the MInt operator',
    )
    parser.add_argument(
        '--sep_ie_layers',
        default=False,
